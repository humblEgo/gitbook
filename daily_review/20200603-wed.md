...
기존에 작성해둔 cub3D 파일을 42seoul norm에 맞게 수정하였다. 함수를 읽으며 평가하는 사람이 main함수에서부터 프로그램 흐름을 따라가기 쉽도록 처음 등장하는 함수의 이름과 그 함수의 이름이 파일명과 일치하도록 신경써보았다. 가령 main함수에서 is\_vaild\_file이라는 함수를 만나면 is\_valid\_file.c 파일을 바로 찾아갈 수 있도록했다. 예전 같으면 valid funciton.c 같은 파일로 퉁쳤을텐데 어떤 방식이 더 좋을지 평가자의 반응을 봐야겠다. 이외에도 함수명과 변수명을 수정하여 결과적으로 아래와 같이 파일을 구성하였다.
